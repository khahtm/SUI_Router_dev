module suinami::amm_script {
    use suinami::amm_swap::{Pool,PoolLiquidityCoin, AdminCap};
    use suinami::amm_config::{GlobalPauseStatus};
    use suinami::amm_router;
    use sui::coin::{Coin};
    use sui::tx_context::{TxContext};

    // T0: Coin_type_A, T1: Coin_type_B 
    // tao pool voi 2 coin_type A va B
    ...
    public entry fun add_liquidity<CoinTypeA, CoinTypeB>(
        pool: &mut Pool<CoinTypeA, CoinTypeB>,  // objectid: pool    
        pause_status: &GlobalPauseStatus,       // objectid: status   Owner: Shared when deploy contract
        coin_a: Coin<CoinTypeA>,                
        coin_b: Coin<CoinTypeB>,                
